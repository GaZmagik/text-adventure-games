// Auto-generated by `tag build-css` — do not edit manually.

export const CDN_BASE = 'https://cdn.jsdelivr.net/gh/GaZmagik/text-adventure-games@817eeae/.claude/skills/text-adventure/assets';

export const CSS_MANIFEST: Record<string, string> = {
  'art-deco': '5bd57708',
  'blueprint': '8942edb4',
  'brutalist': 'e9399598',
  'high-contrast': '31f4eb00',
  'holographic': 'd1a814c8',
  'ink-wash': '6efa405c',
  'neon': '58d080d4',
  'parchment': '60feaa58',
  'stained-glass': '18809840',
  'station': 'dd845450',
  'sveltekit': '73abf631',
  'terminal': '407dfdf8',
  'weathered': '43ef9308',
  'pregame-design': '662e4718',
  'scene-design': '411805c5',
  'common-widget': 'cd6535a0',
};

export const JS_MANIFEST: Record<string, string> = {
  'ta-components.js': '6baa45a6',
  'tag-scene.js': '995f72a8',
  'tag-soundscape.js': '6ec84062',
};

export const ICON_SPRITE_URL = `${CDN_BASE}/icons/sprite.svg?v=ef5f13f8`;
export const ICON_SPRITE_HASH = 'ef5f13f8';
