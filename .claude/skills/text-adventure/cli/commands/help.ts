  ...
  return ok({
    ...
    workflow: {
      ...
      turnLoop: [
        ...
        { step: 3, name: 'Refresh Style', command: 'tag style activate', description: 'Run after setup, after a visual style change, and after compaction. Scene rendering is gated on _styleReadEpoch.' },
        { step: 4, name: 'Render', command: 'tag render scene --style <style>', description: 'Generate complete widget HTML with the custom-element shell, CDN CSS, JS, and interactive elements. Do not hand-alter the shell/runtime scaffolding.' },
        { step: 5, name: 'Compose', command: '(write narrative into #narrative or .scene-phase .narrative)', description: 'Write prose into the skeleton. Follow craftGuidance.proseChecklist and densityGuidance exactly.' },
        { step: 6, name: 'Verify', command: 'tag verify /tmp/scene.html', description: 'Validate composed HTML before show_widget. Checks footer, panels, scene-meta, narrative, CSS, action cards.' },
        { step: 7, name: 'Prose Gate', command: 'tag prose-check /tmp/scene.html', description: 'Run prose review, then clear the gate with `tag prose-gate --manual` or `tag prose-gate --llm /tmp/prose-check-result.json` before show_widget.' },
        { step: 8, name: 'Post-Scene', command: 'tag state sync --apply --scene <N+1>', description: 'Update state: HP, XP, faction standings, world flags, quest progress.' },
      ],
    },
  ...
  return ok({
    ...
    steps: [
      ...
      },
      {
        ...
        description: 'After the player confirms character creation, apply the captured settings + character payload in one call. This replaces the old manual state set flow and persists style, rulebook, modules, pronouns, and computed modifiers.',
      },
      {
        step: 9,
        name: 'Activate Visual Style',
        command: 'tag style activate',
        description: 'Read the active visual style file plus style-reference.md and stamp _styleReadEpoch. tag render scene refuses to run until style guidance is fresh.',
      },
      {
        step: 10,
      ...
      },
      {
        ...
        description: 'Load Tier 2 modules for the chosen scenario and initialise world state from the seed. For each active module, populate the corresponding state: procedural world data, map/nav state, ship/crew rosters, world history, story architect threads, NPCs, quests, factions, and any codex seeds.',
      },
      {
        step: 12,
        name: 'Opening Scene',
  ...
  return ok({
    ...
    workflow: [
      ...
      },
      {
        ...
        name: 'Refresh Style',
        command: 'tag style activate',
        description: 'Run after any visual style change or compaction. tag render scene refuses to run when _styleReadEpoch is stale.',
      },
      {
        step: 4,
        name: 'Render Skeleton',
        command: 'tag render scene --style <style>',
        description: 'Generates complete widget HTML. Keep the custom-element shell, CDN links, JS, and panel/footer scaffolding intact. Compose only inside #narrative, .scene-phase .narrative, and the designated action/POI placeholders before verify.',
      },
      {
        step: 5,
      ...
      },
      {
        ...
        description: 'Validate composed HTML before show_widget. Checks footer buttons, panels, scene-meta, narrative, CSS, and action cards. The verify marker is a signed workflow gate — manual bypass is unsupported and will fail validation.',
      },
      {
        step: 7,
        name: 'Prose Gate',
        command: 'tag prose-check /tmp/scene.html',
        description: 'Run prose review, then clear the scene with `tag prose-gate --manual` or `tag prose-gate --llm /tmp/prose-check-result.json`. If the scene HTML changes, rerun tag prose-check first.',
      },
      {
        step: 8,
