...
const MANUAL_CHECKLIST = [
  {
    ...
    question: 'Do speakers sound distinct beyond utterance length — in vocabulary, rhythm, personality?',
  },
  {
    id: 'exposition_dump',
    question: 'Any 3+ consecutive sentences of backstory with no action beat or sensory grounding?',
  },
...
export async function handleProseCheck(args: string[]): Promise<CommandResult> {
  ...
  try {
    ...
    if (!resolved) {
      return fail(
        ...
        'Path must start with /, ./, ../, or ~/ or end with .html or .htm.',
        'prose-check',
      );
    }
    safePath = resolved;
  ...
  if (mode === 'llm') {
    try {
      ...
      const msg = errorMessage(err);
      return fail(
        `Could not write prose input file: ${msg}`,
        'Check /tmp is writable and has sufficient space.',
        'prose-check',
      );
    }
    return ok(
