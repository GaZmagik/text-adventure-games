...
async function restore(): Promise<CommandResult> {
    ...
    return ok(
      {
        ...
        nextSteps: ['No game state found. Run `tag state reset` to create one.'],
      },
      'compact restore',
    );
  }
  ...
  if (loreSource) {
    nextSteps.push(
      `Lore source on record: ${loreSource}. Run \`tag export load ${shQuote(loreSource)}\` if world data needs reloading.`,
    );
  }
