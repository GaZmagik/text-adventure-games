...
async function defaults(): Promise<CommandResult> {
  ...
  return ok({
    defaults: d,
    ...(d
      ? { message: `Lore defaults: ${Object.entries(d).map(([k, v]) => `${k}=${v}`).join(', ')}` }
      : { message: 'No lore defaults found in state. Run `tag export load <file.lore.md>` to import lore with authored defaults.' }),
  }, 'lore defaults');
}
...
async function pregen(): Promise<CommandResult> {
  ...
  return ok({
    ...
    ...(chars.length > 0
      ...
      : { message: 'No pre-generated characters found in state. Run `tag export load <file.lore.md>` to import lore with authored characters.' }),
  }, 'lore pregen');
}
...
export async function handleLore(args: string[]): Promise<CommandResult> {
  ...
  if (!subcommand) {
    return fail(
      ...
      'Usage: tag lore defaults | pregen | status',
      'lore',
    );
  }
  ...
  switch (subcommand) {
    ...
    default:
      return fail(
        ...
        'Valid subcommands: defaults, pregen, status',
        'lore',
      );
  }
}
