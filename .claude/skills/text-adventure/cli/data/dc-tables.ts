...
export function getDcForLevel(level: number, difficulty: DcDifficulty, gameDifficulty?: GameDifficulty): number {
  const clamped = Math.max(1, Math.min(level, 10));
  const row =
    DC_BY_LEVEL.find(r => clamped >= r.minLevel && clamped <= r.maxLevel) ?? DC_BY_LEVEL[DC_BY_LEVEL.length - 1]!;
