...
export const COMMAND_HELP: Record<TopLevelCommandName, CommandHelp> = {
  ...
  state: {
    ...
    subcommands: [
      ...
      { name: 'codex', usage: 'tag state codex <create|unlock|advance|secret|redact> ...', description: 'Populate lore-codex state with discoverable entries, unlocks, secrets, and redactions', example: 'tag state codex create void-signal --title "Void Signal" --category mystery' },
      { name: 'crew', usage: 'tag state crew <add|morale|stress|loyalty|status|assign> ...', description: 'Populate and mutate crew-manifest state: members, morale, stress, loyalty, status, assignments', example: 'tag state crew add chief_vesk --name "Chief Vesk" --pronouns she/her --role engineer' },
      { name: 'ship', usage: 'tag state ship <init|damage|repair|power|condition|parts> ...', description: 'Populate and mutate ship-systems state: hull/system integrity, power routing, conditions, spare parts', example: 'tag state ship init --name "Wayfarer"' },
