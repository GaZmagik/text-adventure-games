 ...
 */
export function looksLikeSafeReadPath(input: string, extensions: readonly string[] = []): boolean {
 ...
 */
export function resolveSafeReadPath(input: string, options: SafeReadPathOptions): string | null {
  ...
  try {
    ...
    if (code === 'ENOENT' || code === 'ENOTDIR') {
      throw new Error(`${options.kind} file not found: ${input}`, { cause: err });
    }
    throw err;
  }
 ...
 */
export async function readSafeTextFile(filePath: string, kind: string): Promise<string> {
