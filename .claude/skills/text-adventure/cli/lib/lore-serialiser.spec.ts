...
import type { GmState, NpcMutation, Quest, Pronouns, BestiaryTier } from '../types';
import {
...
function makePopulatedState(): GmState {
  ...
  state.character = {
    ...
    proficiencyBonus: 3,
    proficiencies: ['Athletics', 'Religion'],
    abilities: ['Divine Smite'],
    inventory: [{ name: 'Healing Potion', type: 'consumable', slots: 1 }],
    conditions: [],
    equipment: { weapon: 'Longsword', armour: 'Plate' },
  };
  state.rosterMutations = [makeNpc()];
  state.factions = { 'The Guild': 35, 'Shadow Court': -60 };
  state.quests = [makeQuest()];
  state.codexMutations = [{ id: 'codex-1', state: 'discovered', discoveredAt: 5, via: 'investigation' }];
  state.mapState = {
    currentZone: 'Citadel',
    visitedZones: ['Forest', 'Village', 'Citadel'],
  ...
  state._stateHistory = [
    { timestamp: '2026-01-01T00:00:00Z', command: 'test', path: 'scene', oldValue: 0, newValue: 1 },
  ];
...
describe('extractMechanicalData', () => {
  ...
  test('carries generated worldData when present', () => {
    const state = makePopulatedState();
    state.worldData = generateWorld('lore-world', 'space');
    const data = extractMechanicalData(state);
    expect(data.worldData?.seed).toBe('lore-world');
    expect(data.worldData?.rooms[state.worldData.startRoom]).toBeDefined();
  });
  ...
  test('carries arc, arcType, carryForward, arcHistory', () => {
    ...
    state.carryForward = {
      ...
      factionStandings: { 'The Guild': 35 },
      npcDispositions: [],
      codexDiscoveries: ['c1'],
      worldConsequences: ['war'],
    };
    const data = extractMechanicalData(state);
