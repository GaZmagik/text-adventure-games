 ...
 */
function buildFooterFallback(state: GmState | null, dimPanels: string[], hasExport: boolean, hasAudio: boolean): string {
  ...
  if (state?._levelupPending) {
    leftButtons.push('<button class="footer-btn footer-btn-levelup" data-panel="levelup" aria-expanded="false">✦ Level Up</button>');
  }

  if (hasAudio) {
    leftButtons.push('<button class="footer-btn" id="audio-btn" data-sound="ship-engine" data-duration="25">♫ Play</button>');
  }
  ...
  if (hasExport) {
    rightButtons.push(`<button class="footer-btn" id="export-btn" data-prompt="${esc(EXPORT_PROMPT)}" title="${esc(EXPORT_PROMPT)}">Export ↗</button>`);
  }
