...
```json tag-contract
{
  ...
  "kind": "style",
  "version": "1.4.0",
  "summary": "Distressed survival style with rust, concrete, stencilled type, asymmetric borders, and repaired field-interface texture.",
  "mustRead": [
    "Pair with styles/style-reference.md for structural contracts.",
    "Use wear, repair, and salvage cues while keeping controls legible and scannable."
  ],
  "render": [
    "Surfaces should feel used and maintained, not purely ruined.",
    "Best for post-apocalyptic, survival horror, dieselpunk, military thriller, and grimdark scenarios."
  ]
}
```
...
--w-rust-tint: rgba(160, 82, 45, 0.1); /* tinted backgrounds */
--w-rust-glow: rgba(160, 82, 45, 0.2); /* hover fills */
--w-olive: #6b6b47; /* secondary accent — faded olive */
--w-olive-tint: rgba(107, 107, 71, 0.1);
--w-brass: #8b7d3c; /* tarnished brass — decorative highlights */
--w-blood: #5c1a1a; /* dried blood — danger states only */
--w-blood-tint: rgba(92, 26, 26, 0.1);
...
.w-card {
  ...
  clip-path: polygon(0 0, calc(100% - 10px) 0, /* top-right clipped */ 100% 10px, 100% 100%, 0 100%);
...
.root::before {
  ...
  background-image:
    ...
    repeating-linear-gradient(90deg, transparent, transparent 3px, rgba(0, 0, 0, 0.008) 3px, rgba(0, 0, 0, 0.008) 6px);
}
```
...
@media (prefers-reduced-motion: no-preference) {
  ...
  @keyframes w-pulse-border {
    0%,
    100% {
      border-color: var(--w-blood);
    }
    50% {
      border-color: var(--w-blood-tint);
    }
  }
  ...
  .w-scene-enter {
    animation: w-fade-in 0.35s ease-out both;
  }
  .w-data-online {
    animation: w-flicker 0.6s ease-out both;
  }
  .w-warn-pulse {
    animation: w-pulse-border 1.8s ease-in-out infinite;
  }
...
.root {
  ...
  --w-rust: #a0522d;
  --w-rust-dim: #7a3e20;
  --w-rust-tint: rgba(160, 82, 45, 0.1);
  --w-rust-glow: rgba(160, 82, 45, 0.2);
  --w-olive: #6b6b47;
  --w-olive-tint: rgba(107, 107, 71, 0.1);
  --w-brass: #8b7d3c;
  --w-blood: #5c1a1a;
  ...
  --ta-color-warning-bg: rgba(139, 125, 60, 0.1);
